...
CellCycleScoring_fun = function(FRC_input){
  CellCycleScoring(FRC_input, s.features = s.genes, g2m.features = g2m.genes, set.ident = TRUE)
}

temp_list <- lapply(temp_list, CellCycleScoring_fun)
...
sctransform_fun = function(FRC_input){
  SCTransform(FRC_input, verbose = T, vars.to.regress = c("S.Score", "G2M.Score", "percent.mt"))
}
...
GFP_Ag_2D_LP <- NormalizeData(GFP_Ag_2D_LP, assay = "RNA")
GFP_Ag_2D_LP <- RunUMAP(
  ...
  metric = "correlation", 
  assay = "SCT")
GFP_Ag_2D_LP <- FindNeighbors(GFP_Ag_2D_LP, dims = 1:2, verbose = T, reduction = "umap", annoy.metric = "euclidean", n.trees = 100)
GFP_Ag_2D_LP <- FindClusters(GFP_Ag_2D_LP, verbose = T, resolution = 0.05)
DimPlot(GFP_Ag_2D_LP)
...
GFP_Ag_3D_LP <- NormalizeData(GFP_Ag_3D_LP, assay = "RNA")
...
GFP_Ag_3D_LP <- FindNeighbors(GFP_Ag_3D_LP, dims = 1:2, verbose = T, reduction = "umap", annoy.metric = "euclidean", n.trees = 100)
GFP_Ag_3D_LP <- FindClusters(GFP_Ag_3D_LP, verbose = T, resolution = 0.07)
DimPlot(GFP_Ag_3D_LP)
...
Fresh_FRCs <- NormalizeData(Fresh_FRCs, assay = "RNA")
Fresh_FRCs <- RunUMAP(Fresh_FRCs, features = c("Ccl19", "Ccl21a", "Tnfsf11", "Itga7", "Acta2", "Cr2", "Inmt", "Cd34", "Nr4a1", "Il7", "Cxcl9", "Cxcl12", "Cxcl10", "Sox9", "Tmem119", "Pthlh", "Ch25h"), verbose = T, assay = "SCT", min.dist = 0.3, n.epochs = 5000, spread = 1, n.neighbors = 5, metric = "correlation")
Fresh_FRCs <- RunUMAP(
  Fresh_FRCs, 
  dims = 1:17, 
  verbose = TRUE, 
...
Fresh_FRCs <- FindNeighbors(Fresh_FRCs, dims = 1:2,verbose = T, reduction = "umap", annoy.metric = "euclidean", n.trees = 100)
Fresh_FRCs <- FindClusters(Fresh_FRCs, verbose = T, resolution = 0.1)
DimPlot(Fresh_FRCs)
...
Marker_Subset <- dplyr::filter(Fresh_FRCs_Markers, gene %in% c("Ccl19", "Ccl21a", "Tnfsf11", "Itga7", "Acta2", "Cr2", "Inmt", "Cd34", "Nr4a1", "Il7", "Cxcl9", "Cxcl12", "Cxcl10", "Sox9", "Tmem119", "Pthlh", "Ch25h"))
write.csv(Marker_Subset, file = "/home/zachary-wilkes/scRNAseq_Publication/Fresh_FRCs_Markers_subset")
